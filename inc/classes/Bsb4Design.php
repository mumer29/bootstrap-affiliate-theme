...
if (!class_exists('\\BootstrapBasic4\\Bsb4Design')) {
    ...
    class Bsb4Design
    {
        ...
        {
            $output = sprintf('<span class="categories-icon fas fa-th-list" title="' . __('Posted in', 'bootstrap-affiliate') . '"></span> %1$s', $categories_list);
        ...
        {
            $output = __('Continue reading <span class="meta-nav">&rarr;</span>', 'bootstrap-affiliate');
            ...
            if ('pingback' == $comment->comment_type || 'trackback' == $comment->comment_type) { 
                ...
                echo '<div class="comment-body media">';
                    echo '<div class="media-body">';
                        _e('Pingback:', 'bootstrap-affiliate');
                        comment_author_link(); 
                        edit_comment_link(__('Edit', 'bootstrap-affiliate'), '<span class="edit-link">', '</span>');
                    echo '</div>';
                echo '</div>';
            } else {
                echo '<li id="comment-';
                ...
                printf(_x('%1$s at %2$s', '1: date, 2: time', 'bootstrap-affiliate'), get_comment_date(), get_comment_time());
                echo '</time>';
                echo '</a>';
                // end date-time
                ...
                edit_comment_link('<span class="far fa-edit "></span>' . __('Edit', 'bootstrap-affiliate'), '<span class="edit-link">', '</span>');
                echo PHP_EOL;
                ...
                if ('0' == $comment->comment_approved) {
                    echo '<div class="comment-awaiting-moderation text-warning"> <span class="fas fa-info-circle"></span> ';
                        _e('Your comment is awaiting moderation.', 'bootstrap-affiliate');
                    echo '</div>' . PHP_EOL;
                } //endif;
                ...
                printf(__('%s <span class="says">says:</span>', 'bootstrap-affiliate'), sprintf('<cite class="fn">%s</cite>', get_comment_author_link()));
                echo PHP_EOL;
                ...
                comment_reply_link(array_merge($args, [
                    ...
                    'reply_text' => '<span class="fas fa-reply"></span> ' . __('Reply', 'bootstrap-affiliate'),
                    'login_text' => '<span class="fas fa-reply"></span> ' . __('Log in to Reply', 'bootstrap-affiliate')
                ]));
                // end reply link
                echo PHP_EOL;
        ...
        public function editPostLink($return = false)
        {
            ...
            if (null != $edit_post_link) {
                $edit_btn = '<a class="post-edit-link btn btn-light btn-sm" href="'.$edit_post_link.'" title="' . __('Edit', 'bootstrap-affiliate') . '" role="button"><i class="edit-post-icon far fa-edit" title="' . __('Edit', 'bootstrap-affiliate') . '"></i></a>';
        ...
        public function pagination($pagination_align_class = 'justify-content-center', $return = false)
        {
            ...
            if (is_array($pagination_array) && !empty($pagination_array)) {
                $output .= '<nav class="pagination-nav-container" aria-label="'.esc_attr__('Page navigation', 'bootstrap-affiliate').'">';
        ...
        public function postOn($return = false)
        {
            ...
            $output = sprintf(__('<span class="posted-on">Posted on %1$s</span><span class="byline"> by %2$s</span>', 'bootstrap-affiliate'),
                ...
                sprintf('<span class="author vcard"><a class="url fn n" href="%1$s" title="%2$s">%3$s</a></span>',
                    ...
                    esc_attr(sprintf(__('View all posts by %s', 'bootstrap-affiliate'), get_the_author())),
        ...
        {
            $output = sprintf('<span class="tags-icon fas fa-tags" title="' . __('Tagged', 'bootstrap-affiliate') . '"></span> %1$s', $tags_list);
